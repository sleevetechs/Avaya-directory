...
const dbName = process.env.DB_NAME || 'avaya_list';
const config = {
...
async function main() {
  ...
  const hash = CryptoJS.SHA256('1234').toString();
  await conn.query(
    `INSERT IGNORE INTO admins (id, name, email, password_hash, role, is_active)
     VALUES (1, 'Super Admin', 'super-admin@compasslog.com', ?, 'super_admin', 1)`,
    [hash]
  );
  await conn.query(`INSERT IGNORE INTO departments (name) VALUES ('Uncategorised')`);

  const [[{ c: adminCount }]] = await conn.query('SELECT COUNT(*) AS c FROM admins');
  const [[{ c: deptCount }]] = await conn.query('SELECT COUNT(*) AS c FROM departments');
  console.log('Database ready:', dbName);
  console.log('Admins:', adminCount, '| Departments:', deptCount);
  console.log('Login: super-admin@compasslog.com / 1234');
  await conn.end();
}
